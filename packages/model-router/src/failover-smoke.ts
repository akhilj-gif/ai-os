...
// chain-selection and infra-classification rules by permuting env in-process:
// pinned MODEL_PROVIDER = single-element chain (evals stay deterministic);
// unpinned = all configured providers in priority order; only INFRA-class
// failures justify falling through. Run: tsx packages/model-router/src/failover-smoke.ts
import { failoverChain, isInfraFailure, callModel } from './index.js';
...
const setEnv = (env: Record<string, string | undefined>) => {
  for (const k of ['MODEL_PROVIDER', 'ANTHROPIC_API_KEY', 'XAI_API_KEY', 'GEMINI_API_KEY', 'GEMINI_API_KEY_FALLBACK', 'GROQ_API_KEY']) {
    delete process.env[k];
  }
  Object.assign(process.env, env);
};
const names = () => failoverChain().map((p) => p.name).join(',');

console.log('— chain selection —');
setEnv({ GEMINI_API_KEY: 'g1', GROQ_API_KEY: 'q1' });
check('unpinned: gemini then groq (priority order)', names() === 'gemini,groq', names());
...
check('anthropic outranks all when configured', names() === 'anthropic,gemini,groq', names());
...
try {
  const t0 = Date.now();
  const res = await callModel({ role: 'execution', prompt: 'ping', traceId: '00000000-0000-0000-0000-000000000001', name: 'failover-smoke' });
...
try {
  await callModel({ role: 'execution', prompt: 'ping', traceId: '00000000-0000-0000-0000-000000000002', name: 'failover-smoke' });
