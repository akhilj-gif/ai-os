...
- **Baseline still pending — hard external blocker:** Gemini free tier exhausts after ~8–12 calls (confirmed across 6 runs / 2 days); it cannot complete a 9-case run. Closing M2's baseline needs a **paid `MODEL_EXECUTION` key** (e.g. small OpenAI/Anthropic/paid-Gemini tier) or a **local model** (Ollama via the OpenAI-compat shape). Awaiting Akhil's call on which.
- Rolling: failure corpus **20/50** (FC-013..020 from the OS's own runs + reviews); `support-triage` needs ~20 real tickets before its suite is built.
