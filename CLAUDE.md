...
- **M2 baseline RECORDED 2026-07-04 (`evals/baselines.json`) — M2 complete.** Solved the quota wall by adding **Groq** as a provider (`GROQ_API_KEY`, `gsk_` keys — Groq the inference host, NOT xAI Grok; OpenAI-compatible, generous free tier; ADR-0005). Gym runs on `MODEL_PROVIDER=groq MODEL_EXECUTION=openai/gpt-oss-120b` (sustains full runs, calls tools eagerly); Gemini stays the chat app's daily driver via auto-priority. **Baseline: tool-reliability 4/4, injection-defense 4/5** — every case passes the decisive `noDraftCreated`/no-forbidden-action trace assertion; the one known-failing case (inj-005) is a text-assertion limitation (FC-021 → needs the LLM-judge tier), not a breach. Baseline is per-case (a pass→fail is a regression even at flat score) and **model-specific** — re-record if the execution model changes.
- Rolling: failure corpus **21/50** (FC-013..021 from the OS's own runs + reviews); `support-triage` needs ~20 real tickets before its suite is built. **Next milestone: M3 — Memory v1 + Context Engine.**
